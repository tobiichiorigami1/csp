...
def variable_with_weight_loss(shape,stddev,w1):
    ...
    if w1 is not None:
        ...
        tf.add_to_collection('losses',weght_loss)
    return var
...
label_hole = tf.placeholder(tf.int32,[batch_size])
...
local3 = tf.nn.relu(tf.matmul(reshape,weight3)+b3)

weight4 = variable_with_weight_loss(shape=[384,192],stddev=0.04,w1=0.004)
b4 = tf.Variable(tf.constant(0.1,shape =[192]))
local4 = tf.nn.relu(tf.matmul(local3,weight4)+b4)

weight5 = variable_with_weight_loss(shape=[192,10],stddev=1/192.0,w1=0.0)
b5 = tf.Variable(tf.constant(0.0,shape =[10]))
logits = tf.add(tf.matmul(local4,weight5),b5)
